...
class FeedbackAgent(BaseAgent):
    def __init__(self):
        ...
        log_info(
            self.name, "Initialized FeedbackAgent for collecting generation metrics"
        )
    ...
    def _collect_metrics(self, state: State) -> Dict[str, Any]:
        ...
        if metrics["existing_tests_passed"] > 0:
            metrics["test_success_rate"] = (
                metrics["post_tests_passed"] / metrics["existing_tests_passed"]
            )
        else:
            metrics["test_success_rate"] = 0.0

        metrics["code_to_test_ratio"] = metrics["test_length"] / max(
            metrics["code_length"], 1
        )
    ...
    def get_feedback_summary(self) -> Dict[str, Any]:
        ...
        successful_generations = sum(
            1 for m in self.feedback_history if m["has_generated_code"]
        )

        avg_coverage_improvement = (
            sum(m["coverage_improvement"] for m in self.feedback_history)
            / total_generations
        )
        avg_test_improvement = (
            sum(m["tests_improvement"] for m in self.feedback_history)
            / total_generations
        )
        avg_code_length = (
            sum(m["code_length"] for m in self.feedback_history) / total_generations
        )
        avg_test_length = (
            sum(m["test_length"] for m in self.feedback_history) / total_generations
        )
