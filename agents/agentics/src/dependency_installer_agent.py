...
class DependencyInstallerAgent(BaseAgent):
    def __init__(self):
        ...
        self.project_root = os.getenv("PROJECT_ROOT") or "/project"
        self.tools = [
            npm_install_tool,
            read_file_tool,
            write_file_tool,
    ...
    def process(self, state: CodeGenerationState) -> CodeGenerationState:
        ...
        for pkg in npm_packages:
            if isinstance(pkg, dict) and "name" in pkg:
                ...
                # Extract package name from strings like "uuidv7 (UUIDv7 generation)"
                # Take only the first word (the actual package name)
        ...
        if self.tool_executor.execute_tool(
            "check_file_exists_tool", {"file_path": package_json_path}
        ):
            package_json_str = self.tool_executor.execute_tool(
        ...
        for pkg_name in pkgs:
            ...
            try:
                ...
                self.monitor.warning(
                    "install_failed", {"pkg": pkg_name, "error": str(e)}
                )
        ...
        self.monitor.info(
            ...
            {"total": len(pkgs), "installed": len(installed), "pkgs": installed},
        )
        log_info(
            self.name, f"Installed {len(installed)}/{len(pkgs)} packages: {installed}"
        )
        return state
