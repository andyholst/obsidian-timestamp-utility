...
def parse_json_response(
    ...
    if JSON_REPAIR_AVAILABLE:
        parsing_attempts.append(
            ("json_repair", lambda: json.loads(repair_json(cleaned_response)))
        )
    parsing_attempts.append(
    ...
    for attempt_name, parse_func in parsing_attempts:
        try:
            ...
            if required_keys:
                result = _validate_and_fill_json(
                    result, required_keys, fallback_defaults or {}
                )
                monitor.info(
                    "json_validation_passed",
                    data={"required_keys": list(required_keys)},
                )

            return result
        except (json.JSONDecodeError, ValueError) as e:
            monitor.warning(
                "json_parsing_attempt_failed",
                data={"method": attempt_name, "error": str(e)},
            )
            last_error = e
            continue
    ...
    if llm_client and original_prompt:
        for retry in range(max_retries):
            monitor.info(
                "json_parsing_retry",
                data={"retry": retry + 1, "max_retries": max_retries},
            )
            feedback_prompt = (
                original_prompt
            ...
            try:
                ...
                for attempt_name, parse_func in parsing_attempts:
                    try:
                        result = parse_func()
                        monitor.info(
                            "json_parsing_success_on_retry",
                            data={"method": attempt_name, "retry": retry + 1},
                        )
                        if required_keys:
                            result = _validate_and_fill_json(
                                result, required_keys, fallback_defaults or {}
                ...
                monitor.warning(
                    "json_parsing_retry_failed",
                    data={"retry": retry + 1, "error": str(e)},
                )
                continue
    ...
    try:
        ...
        # Strict validation: raise if no meaningful content extracted
        if not result or (isinstance(result, dict) and len(result) == 0):
            raise ValueError(
                f"No valid JSON content extracted from response (empty dict fallback)"
            )
        return result
    except Exception as e:
        monitor.error(
            "json_parsing_all_failed",
            data={"last_error": str(last_error), "full_response": response},
        )
        raise ValueError(
            f"LLM did not return a valid JSON object after trying all parsing methods and retries. Last error: {str(last_error)}"
        )
...
def _extract_fields_by_regex(text: str) -> dict:
    ...
    monitor.info(
        "regex_field_extraction", data={"extracted_fields": list(fields.keys())}
    )
    return fields


def _validate_and_fill_json(
    parsed_json: dict, required_keys: set, defaults: dict
) -> dict:
    ...
    if missing_keys:
        ...
        for key in missing_keys:
            if key in defaults:
                ...
                monitor.info(
                    "json_key_filled_with_default",
                    data={"key": key, "default": defaults[key]},
                )
            else:
                raise ValueError(
                    f"Required key '{key}' is missing and no default provided"
                )
...
def safe_json_dumps(obj, indent=None, **kwargs):
    ...
    try:
        ...
        monitor.warning(
            ...
            data={"error": str(e), "object_type": type(obj).__name__},
        )
        return json.dumps(
            {
