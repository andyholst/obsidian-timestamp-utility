...
class TicketClarityAgent(BaseAgent):
    ...
    def process(self, state: State) -> State:
        ...
        log_info(self.name, f"Ticket content length: {len(ticket_content)}")

        # Use simplified single-call approach for reliability
        refined_ticket = self._simplify_and_extract(ticket_content)
...
{{"title":"short title","description":"brief description","requirements":["req1","req2","req3","req4","req5"],"acceptance_criteria":["ac1","ac2"],"implementation_steps":["step1","step2"],"npm_packages":[],"affected_files":["src/main.ts"]}}"""
        ...
        try:
            ...
            log_info(self.name, f"JSON parsing failed: {e}, using fallback")

        # Fallback: extract requirements from plain text using simple heuristics
        return self._fallback_extract(ticket_content)

    def _fallback_extract(self, ticket_content: str) -> Dict:
        """Extract requirements from ticket content using simple heuristics."""
        # Split by lines and look for requirement-like patterns
    ...
    # Keep old methods for backward compatibility but they're no longer used
    def refine_ticket(self, ticket_content):
        """Deprecated: use _simplify_and_extract instead."""
        return self._fallback_extract(ticket_content)
        for iteration in range(self.max_iterations):
            log_info(
                self.name,
                f"Refinement iteration {iteration + 1} of {self.max_iterations}",
            )
            ...
            log_info(
                self.name, f"Ticket string length for evaluation: {len(ticket_str)}"
            )
            ...
            if (
                ...
                or "is_clear" not in evaluation
                or "suggestions" not in evaluation
            ):
            ...
            if evaluation["is_clear"]:
                ...
                break
            refined_ticket = self.generate_improvements(
                ticket_str, evaluation, original_content=str(original_ticket_content)
            )
            log_info(
                self.name,
                f"Refined ticket after improvements: {json.dumps(refined_ticket, indent=2)}",
            )
            # Validate refined ticket response
            if not isinstance(refined_ticket, dict) or not all(
                key in refined_ticket
                for key in [
                    "title",
                    "description",
                    "requirements",
                ...
                self.monitor.warning(
                    f"Invalid refined ticket response, setting defaults: {refined_ticket}"
                )
                refined_ticket = {
                    "title": refined_ticket.get("title", "Feature Implementation"),
                    "description": refined_ticket.get(
                        "description", "Derived from ticket."
                    ),
                    "requirements": refined_ticket.get("requirements", []),
                    "acceptance_criteria": refined_ticket.get(
                        "acceptance_criteria", []
                    ),
                    "implementation_steps": refined_ticket.get(
        ...
        ticket_str = (
            json.dumps(refined_ticket)
            if isinstance(refined_ticket, dict)
            else str(refined_ticket)
        )
        ...
        refined_ticket = self.generate_improvements(
            ticket_str,
            {"suggestions": extraction_suggestions},
            original_content=str(original_ticket_content),
        )
        log_info(
            self.name, f"Refined ticket result: {json.dumps(refined_ticket, indent=2)}"
        )
        ...
        if len(reqs) < 3:
            log_info(
                self.name,
                f"Requirements count {len(reqs)} < 3, triggering full re-refine with original appended",
            )
        ...
        if len(reqs) < 5:
            log_info(
                self.name, f"Requirements count {len(reqs)} < 5, applying fallback"
            )
            refined_ticket["requirements"] = self._generate_fallback_requirements(
                refined_ticket, str(original_ticket_content)
            )
        if len(acs) < 3:
            log_info(
                self.name,
                f"Acceptance criteria count {len(acs)} < 3, triggering full re-refine",
            )
            re_refine_prompt = (
                ticket_str
        ...
        if len(acs) < 5:
            log_info(
                self.name,
                f"Acceptance criteria count {len(acs)} < 5, applying fallback",
            )
            refined_ticket["acceptance_criteria"] = (
                self._generate_fallback_acceptance_criteria(
                    refined_ticket, str(original_ticket_content)
                )
            )
        log_info(
            self.name,
            f"Final requirements length: {len(refined_ticket.get('requirements', []))}, AC length: {len(refined_ticket.get('acceptance_criteria', []))}",
        )
        return refined_ticket

    def evaluate_clarity(self, ticket_content):
        log_info(self.name, "Evaluating ticket clarity")
        prompt = ModularPrompts.get_ticket_clarity_evaluation_prompt().format(
        ...
        result = parse_json_response(
            clean_response, llm_client=self.llm, original_prompt=prompt
        )
    ...
    def generate_improvements(self, ticket_content, evaluation, original_content=None):
        ...
        prompt = ModularPrompts.get_ticket_clarity_improvements_prompt().format(
            ticket_content=ticket_content, suggestions="\n".join(suggestions)
        )
        ...
        clean_response = remove_thinking_tags(response)
        log_info(
            self.name, f"Raw LLM response for generate_improvements: {clean_response}"
        )
        result = parse_json_response(
            clean_response, llm_client=self.llm, original_prompt=prompt
        )
        ...
        result.setdefault(
            "description", "Detailed feature description derived from ticket."
        )
        ...
        if not result.get("title", "").strip() and original_content:
            ...
            if title_match:
                ...
                if lines:
                    result["title"] = lines[0].strip()[:60] + (
                        "..." if len(lines[0]) > 60 else ""
                    )
        if not result.get("description", "").strip() and original_content:
            desc_preview = original_content.strip()[:400] + (
                "..." if len(original_content) > 400 else ""
            )
            result["description"] = desc_preview
        log_info(
            self.name,
            f"Post-parse enforced: title_len={len(result.get('title', ''))}, desc_len={len(result.get('description', ''))}, reqs_len={len(result.get('requirements', []))}, ac_len={len(result.get('acceptance_criteria', []))}",
        )
    ...
    def _merge_original_content(self, refined: dict, original: str) -> dict:
        ...
        if not refined.get("requirements") or len(refined["requirements"]) < 3:
            keywords = re.findall(
                r"\b(UUID v7|timestamp|crypto\.random|Notice|editor\.replaceSelection|command palette)\b",
                original,
                re.IGNORECASE,
            )
            if keywords:
                refined.setdefault("requirements", []).append(
                    f"Incorporate original specifications: {', '.join(set(keywords))}"
                )
        # Similarly for acceptance_criteria
        if (
            not refined.get("acceptance_criteria")
            or len(refined["acceptance_criteria"]) < 3
        ):
            keywords = re.findall(
                r"\b(command visible|inserts correctly|shows error|no note|validation)\b",
                original,
                re.IGNORECASE,
            )
            if keywords:
                refined.setdefault("acceptance_criteria", []).append(
                    f"Ensure original requirements: {', '.join(set(keywords))}"
                )
    ...
    def _generate_fallback_requirements(
        self, ticket: dict, original_content: str = None
    ) -> list:
        ...
        if original_content:
            keywords = re.findall(
                r"\b(UUID v7|timestamp|crypto\.random|Notice|editor\.replaceSelection|command palette|public method|uniqueness|validation|edge cases|testable)\b",
                original_content,
                re.IGNORECASE,
            )
        ...
        if keywords and not any(k.lower() in " ".join(reqs).lower() for k in keywords):
            for k in keywords[:2]:  # add up to 2
                reqs.append(
                    f"Generate UUID v7 using timestamp as described in original: {k}"
                )
        defaults = [
            "Implement as Obsidian command",
            "Add public method",
            "Handle errors",
            "Add types",
    ...
    def _generate_fallback_acceptance_criteria(
        self, ticket: dict, original_content: str = None
    ) -> list:
        ...
        if original_content:
            keywords = re.findall(
                r"\b(command visible|inserts correctly|shows error|no note|validation|success notice|error notice|compilation)\b",
                original_content,
                re.IGNORECASE,
            )
            keywords = list(set(keywords))  # unique
        ...
        defaults = [
            "Command visible in palette",
            "Inserts correctly at cursor",
            "Shows success notice",
            "Shows error notice if invalid",
    ...
    def post_final_ticket(self, refined_ticket, issue_url):
        ...
        self.monitor.info(
            "post_final_ticket_keys",
            {
                "keys": list(refined_ticket.keys())
                if isinstance(refined_ticket, dict)
                else "non-dict",
