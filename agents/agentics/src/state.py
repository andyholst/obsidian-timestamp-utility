...
class CodeGenerationState:
    ...
    def with_code(
        ...
        return CodeGenerationState(
            **{
                ...
                if k not in ["generated_code", "method_name", "command_id"]
            },
            generated_code=code,
    ...
    def with_post_metrics(
        self,
        tests_passed: int,
        coverage_all_files: float,
        existing_tests_passed: int,
        existing_coverage_all_files: float,
    ) -> "CodeGenerationState":
    ...
    def with_recovery_update(
        self, confidence: float, explanation: str
    ) -> "CodeGenerationState":
        ...
        return CodeGenerationState(
            **{
                k: v
                for k, v in self.__dict__.items()
                if k not in ["recovery_confidence", "recovery_explanation"]
            },
            recovery_confidence=confidence,
            recovery_explanation=explanation,
        )
    ...
    def get(self, key: str, default=None):
        """Dict-like .get() for compatibility with code expecting dict state."""
