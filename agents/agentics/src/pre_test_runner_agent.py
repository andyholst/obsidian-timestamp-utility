...
class PreTestRunnerAgent(BaseAgent):
    ...
    def process(self, state: State) -> State:
        ...
        log_info(
            self.name,
            f"Current user: {os.getuid() if hasattr(os, 'getuid') else 'unknown'}",
        )
        ...
        if self.install_command == "npm install" and self.test_command == "npm test":
            ...
            if not exists:
                ...
                self.monitor.info(
                    f"Extracted default metrics: tests_passed=0, coverage_all_files=0.0"
                )
                self.monitor.info(
            ...
            try:
                ...
                try:
                    ...
                    if result == 0:
                        ...
                        log_info(
                            self.name,
                            f"chmod command failed with exit code {result}, continuing anyway",
                        )
                except Exception as chmod_e:
                    log_info(
                        self.name,
                        f"Failed to fix permissions: {chmod_e}, continuing anyway",
                    )
        ...
        def run_install():
            ...
            try:
                ...
                log_info(
                    self.name, f"Could not set project directory permissions: {perm_e}"
                )
            ...
            log_info(
                self.name, f"Updated PATH to include /usr/bin: {os.environ['PATH']}"
            )
            ...
            if (
                install_cmd[0] == "npm"
                and len(install_cmd) >= 2
                and install_cmd[1] == "install"
            ):
            ...
            if install_result.stderr:
                log_info(
                    self.name, f"Install stderr length: {len(install_result.stderr)}"
                )
        ...
        try:
            tsc_result = self.tool_executor.execute_tool(
        ...
        for attempt in range(max_retries):
            try:
                log_info(
                    self.name,
                    f"Running test command (attempt {attempt + 1}/{max_retries}): {self.test_command}",
                )
                ...
                combined_output = self.strip_ansi_codes(
                    test_result.stdout + test_result.stderr
                )
                log_info(self.name, f"Test command completed successfully")
                log_info(
                    self.name, f"Combined test output length: {len(combined_output)}"
                )
                ...
                self.monitor.error(
                    f"Test command failed on attempt {attempt + 1}/{max_retries} with return code {e.returncode}"
                )
                ...
                if attempt < max_retries - 1:
                    ...
                    self.monitor.info(
                        f"Retrying test command in {wait_time} seconds..."
                    )
                    ...
                    log_file = os.path.join(
                        self.project_root,
                        "results",
                        f"test_failure_pre_{timestamp}.log",
                    )
        ...
        self.monitor.info(
            f"Extracted metrics: tests_passed={tests_passed}, coverage_all_files={coverage_all_files}"
        )
        self.monitor.info(
            f"After processing in {self.name}: {safe_json_dumps(state, indent=2)}"
        )
