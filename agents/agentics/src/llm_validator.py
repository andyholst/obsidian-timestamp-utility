...
class LLMResponseValidator:
    ...
    def __init__(self):
        ...
        self.enable_semantic_validation = (
            os.getenv("ENABLE_SEMANTIC_VALIDATION", "true").lower() == "true"
        )

    def validate_response(
        self, response: str, response_type: str, context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            quality_score = self._calculate_quality_score(
                response, response_type, context
            )
    ...
    def _validate_structure(
        self, response: str, response_type: str, result: Dict[str, Any]
    ):
    ...
    def _validate_code_structure(self, response: str, result: Dict[str, Any]):
        ...
        has_code_patterns = any(
            re.search(pattern, response) for pattern in code_indicators
        )
    ...
    def _calculate_quality_score(
        self, response: str, response_type: str, context: Dict[str, Any]
    ) -> float:
        ...
        self.monitor.debug(
            f"Quality scores: {[(name, score) for name, score, _ in scores]} = {final_score:.3f}"
        )
    ...
    def _score_content_richness(self, response: str, response_type: str) -> float:
        ...
        if response_type == "json":
            ...
            found_constructs = sum(
                1 for pattern in constructs if re.search(pattern, response)
            )
    ...
    def _score_context_relevance(self, response: str, context: Dict[str, Any]) -> float:
        ...
        if "domain" in context:
            ...
            if expected_terms:
                ...
                relevance_indicators += sum(
                    1 for term in expected_terms if term in response
                )
    ...
    def _validate_code_semantics(self, response: str, result: Dict[str, Any]):
        ...
        if self._is_typescript_code(response):
            ...
            result["warnings"].append(
                "Unable to determine code language for semantic validation"
            )
...
"""
            ...
            if compile_result.returncode != 0:
                ...
                if errors:
                    result["issues"].append(
                        f"TypeScript compilation errors: {errors[:200]}..."
                    )
            ...
            if isinstance(parsed, dict):
                ...
                if empty_fields:
                    result["warnings"].extend(
                        f"Empty field: {field}" for field in empty_fields
                    )
        except:
            ...
            for pattern in dangerous_patterns:
                if re.search(pattern, sanitized):
                    self.monitor.warning(
                        f"Potentially dangerous pattern detected in code: {pattern}"
                    )
...
class MultiModelFallback:
    ...
    def invoke_with_fallback(self, prompt: str, **kwargs) -> str:
        ...
        models_to_try = [self.primary_llm] + [
            config["llm"] for config in self.fallback_llms
        ]

        for i, llm in enumerate(models_to_try):
            try:
                self.monitor.info(
                    f"Trying model {i + 1}/{len(models_to_try)}: {getattr(llm, 'model', 'unknown')}"
                )
...
def validate_llm_response(
    response: str, response_type: str = "text", context: Dict[str, Any] = None
) -> Dict[str, Any]:
