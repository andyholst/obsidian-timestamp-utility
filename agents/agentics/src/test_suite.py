...
class ValidationResult:
    ...
    test_execution: ExecutionMetrics = field(default_factory=ExecutionMetrics)

    # Relationship Analysis
    test_code_relationship: CodeRelationship = field(default_factory=CodeRelationship)

    # Pattern Compliance
    langchain_compliance: LangChainCompliance = field(
        default_factory=LangChainCompliance
    )
...
class SuiteExecutor:
    ...
    ) -> Tuple[CodeExecutionMetrics, ExecutionMetrics]:
...
"""
                ...
                memory_limit = context.get("sandbox_config", {}).get(
            ...
            return CodeExecutionMetrics(
                success=False, execution_time=time.time() - start_time, error_count=1
            )

    def _execute_tests(
        self, test_code: str, source_code: str, context: Dict[str, Any]
    ) -> ExecutionMetrics:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                print(
                    f"DEBUG_COVERAGE: coverage_dir_exists={os.path.exists(coverage_dir)}"
                )
                ...
                # Parse Jest output
                metrics = self._parse_jest_results(
                    result.stdout, result.stderr, temp_dir
                )
            ...
            return ExecutionMetrics(
                execution_time=time.time() - start_time, error_messages=[str(e)]
            )
    ...
    def _extract_exports(self, code: str) -> str:
        ...
        export_matches = re.findall(
            r"export\s+(?:class|function|const|let|var)\s+(\w+)", code
        )
    ...
    def _parse_jest_results(
        self, stdout: str, stderr: str, temp_dir: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if stdout:
                ...
                num_skipped_tests = result_data.get(
                    "numPendingTests", 0
                ) + result_data.get("numSkippedTests", 0)
                ...
                if not detailed_parsed:
                    metrics["total"] = (
                        num_total_tests + num_runtime_errors + num_failed_suites
                    )
                    metrics["passed"] = num_passed_tests
                    metrics["failed"] = (
                        num_failed_tests_summary
                        + num_runtime_errors
                        + num_failed_suites
                    )
                ...
                if stderr.strip():
                    ...
                    for line in stderr.splitlines()[-10:]:
                        ...
                        if stripped and (
                            ...
                            or "referenceerror" in stripped.lower()
                        ):
            ...
            print(
                f"DEBUG_PARSE: coverage_file={coverage_file}, exists={os.path.exists(coverage_file)}"
            )
            ...
            for line in lines:
                if "Tests:" in line and "passed" in line:
                    match = re.search(
                        r"Tests:\s*(\d+)\s*passed,\s*(\d+)\s*failed", line
                    )
                    if match:
...
class AssertionValidator:
    ...
    def _categorize_assertions(self, ast_tree: ast.Module) -> Dict[str, int]:
        ...
        class CategorizationVisitor(ast.NodeVisitor):
            def visit_Assert(self, node):
                ...
                elif isinstance(node.test, ast.Name) or isinstance(
                    node.test, ast.Attribute
                ):
        ...
        class MockVisitor(ast.NodeVisitor):
            ...
            def visit_Call(self, node):
                ...
                if isinstance(node.func, ast.Name):
                    ...
                    if "mock" in node.func.attr.lower() or node.func.attr in [
                        "patch",
                        "Mock",
                        "MagicMock",
                    ]:
    ...
    def _analyze_jest_structure(self, test_code: str) -> Dict[str, Any]:
        ...
        structure = {
            ...
            "setup_methods": len(
                re.findall(r"beforeEach\s*\(|beforeAll\s*\(", test_code)
            ),
            "teardown_methods": len(
                re.findall(r"afterEach\s*\(|afterAll\s*\(", test_code)
            ),
...
class SuiteValidator:
    ...
    def validate_test_code_relationship(
        self, test_code: str, source_code: str, test_metrics: ExecutionMetrics
    ) -> CodeRelationship:
    ...
    def _categorize_tests(self, test_code: str) -> Dict[str, int]:
        ...
        unit_tests = len(
            re.findall(r"it\s*\(", test_code)
        )  # All 'it' blocks are unit tests
        integration_tests = len(
            re.findall(
                r"describe\(.*integration|describe\(.*e2e", test_code, re.IGNORECASE
            )
        )
        log_info(
            __name__,
            f"Test categorization - unit matches: {unit_tests}, integration matches: {integration_tests}, test_code preview: {test_code[:100]}",
        )

        return {"unit": unit_tests, "integration": integration_tests}
...
class LangChainBestPracticesValidator:
    ...
    def _analyze_error_handling(self, code: str) -> float:
        ...
        error_handling_found = sum(
            1 for pattern in error_patterns if re.search(pattern, code)
        )
    ...
    def _analyze_state_management(self, code: str) -> float:
        ...
        transform_patterns = [r"with_", r"transform", r"update", r"merge", r"combine"]

        transforms_found = sum(
            1
    ...
    def _calculate_composability(self, code: str) -> float:
        ...
        modular_found = sum(
            1 for pattern in modular_patterns if re.search(pattern, code)
        )
        score += min(5.0, modular_found * 1.5)  # More generous

        print(
            f"DEBUG: Composability score: {score}, composition_found: {composition_found}, modular_found: {modular_found}"
        )
        return min(10.0, score)
...
class SuiteReporter:
    ...
    def generate_comprehensive_report(
        ...
        if code_validator_report:
            report_sections.append(
                self._generate_code_validator_section(code_validator_report)
            )
        ...
        # Recommendations
        report_sections.append(
            self._generate_recommendations_section(validation_result)
        )
    ...
    def _generate_header(self, result: ValidationResult) -> str:
...
"""
    ...
    def _generate_execution_section(self, result: ValidationResult) -> str:
        ...
        log_info(
            __name__,
            f"Generating execution section - total_tests: {total_tests}, passed: {passed_tests}",
        )
...
"""
    ...
    def _generate_relationship_section(self, result: ValidationResult) -> str:
...
"""
    ...
    def _generate_compliance_section(self, result: ValidationResult) -> str:
...
"""
    ...
    def _generate_recommendations_section(self, result: ValidationResult) -> str:
...
class LLMSuiteValidator:
    ...
    def validate_test_suite(
        ...
        try:
            ...
            result.test_code_relationship = (
                self.test_validator.validate_test_code_relationship(
        ...
        try:
            ...
            result.langchain_compliance = (
                self.langchain_validator.validate_langchain_compliance(code)
            )
        except Exception as e:
    ...
    def _calculate_overall_score(
        self, result: ValidationResult, code_validator_report: ValidationReport = None
    ) -> float:
        ...
        # Weighted average
        weights = {"execution": 0.4, "relationship": 0.3, "compliance": 0.3}
    ...
    def _score_execution_results(self, result: ValidationResult) -> float:
        """Score execution results (0-100)"""
        score = 0.0
        log_info(
            __name__,
            f"Scoring execution - total_tests: {result.test_execution.total_tests}, passed: {result.test_execution.passed_tests}",
        )
        ...
        if result.test_execution.total_tests > 0:
            pass_rate = (
                result.test_execution.passed_tests / result.test_execution.total_tests
            )
            score += pass_rate * 40.0
            log_info(
                __name__, f"Pass rate: {pass_rate}, score addition: {pass_rate * 40.0}"
            )
        else:
    ...
    def _score_relationship(self, result: ValidationResult) -> float:
    ...
    def _generate_recommendations(
        ...
        if not result.code_execution.success:
            result.critical_issues.append(
                "Generated code execution failed - review syntax and dependencies"
            )

        if result.test_execution.failed_tests > 0:
            result.critical_issues.append(
                f"{result.test_execution.failed_tests} tests failed - fix test logic"
            )

        if result.test_execution.coverage_percentage < 50:
            result.critical_issues.append(
                "Test coverage below 50% - add more comprehensive tests"
            )

        # Warnings
        if not result.test_code_relationship.mock_usage:
            result.warnings.append(
                "No mock usage detected - consider adding mocks for external dependencies"
            )

        if result.test_code_relationship.edge_case_coverage < 3:
            result.warnings.append(
                "Limited edge case coverage - add tests for boundary conditions"
            )

        if result.langchain_compliance.overall_compliance < 5.0:
            result.warnings.append(
                "Low LangChain best practices compliance - review patterns"
            )

        # Suggestions
        if result.test_code_relationship.assertion_quality < 5.0:
            result.suggestions.append(
                "Improve assertion quality with more diverse test assertions"
            )
        ...
        if result.langchain_compliance.error_handling_score < 5.0:
            result.suggestions.append(
                "Enhance error handling patterns with try-catch blocks and circuit breakers"
            )


# Global instance
test_suite_validator = LLMSuiteValidator()
...
# Aliases for backward compatibility with test imports
TestSuiteExecutor = SuiteExecutor
SuiteValidationResult = ValidationResult
TestSuiteValidationResult = ValidationResult
SuiteRiskLevel = RiskLevel
TestSuiteRiskLevel = RiskLevel
TestExecutionMetrics = ExecutionMetrics
LLMTestSuiteValidator = LLMSuiteValidator
TestCodeRelationship = CodeRelationship
