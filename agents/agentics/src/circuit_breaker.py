...
class CircuitBreaker:
    ...
    def __init__(
        ...
        monitor.info(
            ...
            data={
                ...
                "has_fallback": fallback_strategy is not None,
            },
        )
    ...
    def _record_success(self):
        ...
        if self.state == CircuitBreakerState.HALF_OPEN:
            ...
            monitor.info(
                ...
                data={"name": self.name, "success_count": self.success_count},
            )
    ...
    def _record_failure(self):
        ...
        if self.state == CircuitBreakerState.HALF_OPEN:
            monitor.warning(
                "circuit_breaker_failure_half_open", data={"name": self.name}
            )
            self._open_circuit()
        elif self.failure_count >= self.failure_threshold:
            monitor.warning(
                "circuit_breaker_threshold_reached",
                data={"name": self.name, "failure_count": self.failure_count},
            )
            self._open_circuit()
        ...
        if self.state == CircuitBreakerState.OPEN:
            if not self._should_attempt_reset():
                ...
                if self.fallback_strategy:
                    monitor.warning(
                        "circuit_breaker_open_using_fallback", data={"name": self.name}
                    )
                    try:
                        ...
                        monitor.error(
                            ...
                            data={"name": self.name, "error": str(fallback_error)},
                        )
                        raise CircuitBreakerOpenException(
                            f"CircuitBreaker '{self.name}' is OPEN and fallback failed"
                        )
                else:
                    raise CircuitBreakerOpenException(
...
class ServiceHealthMonitor:
    ...
    def check_service_health(self, name: str) -> bool:
        ...
        try:
            ...
            monitor.error(
                ...
                data={"service_name": name, "error": str(e)},
            )
            return False
    ...
    def attempt_graceful_degradation(self, service_name: str, *args, **kwargs) -> Any:
        ...
        if service_name in self.degradation_strategies:
            monitor.info(
                "graceful_degradation_attempt", data={"service_name": service_name}
            )
            try:
            ...
            monitor.warning(
                "no_degradation_strategy", data={"service_name": service_name}
            )
            raise RuntimeError(
                f"Service '{service_name}' is unavailable and no degradation strategy exists"
            )
