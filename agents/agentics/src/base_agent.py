...
class BaseAgent(Runnable[State, State]):
    ...
    def invoke(self, input: State, config: Optional[RunnableConfig] = None) -> State:
        return self(input)
    ...
    def _monitored_process(self, state: State) -> State:
    ...
    def _create_error_context(self, state: State, exception: Exception) -> Dict[str, Any]:
        ...
        return {
            ...
            "state_snapshot": {
                "url": state.get("url"),
                "ticket_content_length": len(state.get("ticket_content", "")),
                "result_keys": list((state.get("result") or {}).keys()),
                "generated_code_length": len(state.get("generated_code") or ""),
                "generated_tests_length": len(state.get("generated_tests") or ""),
                "relevant_files_count": len(state.get("relevant_code_files", [])) + len(state.get("relevant_test_files", []))
            },
