...
def _post_process_generated_code(code: str) -> str:
    ...
    auto-fix common LLM errors (const reassignment, Node.js APIs)."""
    ...
    # Auto-fix Node.js Buffer.from() → browser crypto.getRandomValues()
    code = re.sub(r'Buffer\.from\(\[([^\]]*)\]\)\.toString\([\'"]hex[\'"]\)',
...
class AgenticsWorkflow:
    ...
    def _node_generate_code_tests(self, state: State) -> State:
        ...
        state["_persisted_export_name"] = export_name
        state["_persisted_command_id"] = command_id
        gen_file = os.path.join(gen_dir, f"{slug}.ts")
        gen_test_file = os.path.join(gen_test_dir, f"{slug}.test.ts")
