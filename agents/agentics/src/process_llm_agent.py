...
class ProcessLLMAgent(BaseAgent):
    ...
    def _create_processing_chain(self):
        ...
        return (
            RunnablePassthrough.assign(ticket_content=self._prepare_ticket_content)
    ...
    def _prepare_ticket_content(self, inputs):
        ...
        log_info(
            self.name,
            f"Ticket content source: {'refined_ticket' if 'refined_ticket' in state else 'ticket_content'}",
        )
    ...
    def _process_response(self, response):
        ...
        if not isinstance(result["manual_implementation_notes"], str):
            result["manual_implementation_notes"] = str(
                result["manual_implementation_notes"]
            )
        ...
        validation_result = validate_llm_response(
            json.dumps(result), "json", validation_context
        )

        if not validation_result["is_valid"]:
            log_info(
                self.name,
                f"LLM response validation failed: {validation_result['issues']}",
            )
        ...
        log_info(
            self.name,
            f"Valid result parsed and validated: {json.dumps(result, indent=2)}",
        )
        return result
    ...
    def process(self, state: State) -> State:
        ...
        if "refined_ticket" in state:
            ...
            if isinstance(refined, dict) and all(
                key in refined
                for key in [
                    "title",
                    "description",
                ...
                log_info(
                    self.name,
                    f"Bypassed LLM re-processing using refined_ticket. Reqs len: {len(refined.get('requirements', []))}, AC len: {len(refined.get('acceptance_criteria', []))}",
                )
                log_info(
                    self.name,
                    f"After bypass in {self.name}: {safe_json_dumps(state, indent=2)}",
                )
                return state
        ...
        if cached_result:
            ...
            self.memory_manager.track_object(
                f"cached_result_{cache_key}", cached_result
            )
        ...
        for attempt in range(self.max_retries):
            log_info(
                self.name,
                f"Attempt {attempt + 1} of {self.max_retries} to invoke LLM chain",
            )
            try:
                try:
                    result = get_circuit_breaker("llm_processing").call(
                        lambda: self.processing_chain.invoke(state)
                    )
                except CircuitBreakerOpenException as e:
                    self.monitor.error(
                        f"Circuit breaker open for LLM processing: {str(e)}"
                    )
                    raise
                except Exception as e:
                ...
                if attempt == self.max_retries - 1:
                    self.monitor.error(
                        f"Invalid LLM response after {self.max_retries} attempts: {str(e)}"
                    )
                    raise ValueError(
                        f"Invalid LLM response after {self.max_retries} attempts: {str(e)}"
                    )
