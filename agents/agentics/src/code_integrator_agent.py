...
class CodeIntegratorAgent(ToolIntegratedAgent):
    def __init__(self, llm_client):
        super().__init__(
            llm_client,
            [read_file_tool, check_file_exists_tool, write_file_tool],
            "CodeIntegrator",
        )
    ...
    def process(self, state: State) -> State:
        ...
        try:
            ...
            if relevant_code_files or relevant_test_files:
                ...
                new_code_file = os.path.join(
                    self.project_root, "src", f"{filename}{self.code_ext}"
                )
                new_test_file = os.path.join(
                    self.project_root, "src", "__tests__", f"{filename}{self.test_ext}"
                )
    ...
    def generate_filename(self, task_description: str, task_title: str) -> str:
        ...
        try:
            ...
            if (
                re.match(r"^[a-zA-Z0-9_]+$", suggested_filename)
                and len(suggested_filename.split()) == 1
            ):
