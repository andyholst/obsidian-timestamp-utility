...
def main():
    parser = argparse.ArgumentParser(
        description="Parse executed tests from pytest output"
    )
    ...
    input_file = f"results/executed_tests_{args.type}.txt"
    ...
    collected_file = f"results/collected_tests_{args.type}.txt"
    ...
    if passed_match and int(passed_match.group(1)) == expected_count:
        output_file = f"executed_list_{args.type}.txt"
        ...
        print(
            f"Not all tests passed ({passed_count}/{expected_count}). Left {output_file} empty.",
            file=sys.stderr,
        )
        # sys.exit(1)  # Disabled for CI stability
