...
class PostTestRunnerAgent(BaseAgent):
    ...
    def process(self, state: State) -> State:
        ...
        self.logger.info("Starting post-test runner process")
        ...
        try:
            ...
            raise RuntimeError(f"Post-integration tests failed: {e.stderr}")
        ...
        self.logger.info(f"Post-integration metrics: tests_passed={tests_passed}, coverage_all_files={coverage_all_files}")
        self.logger.info(f"Improvement: coverage +{coverage_improvement:.2f}%, tests +{tests_improvement}")
