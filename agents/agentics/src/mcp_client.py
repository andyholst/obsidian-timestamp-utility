...
class MCPClient:
    def __init__(self, bridge_url: str = None):
        if bridge_url is None:
            bridge_url = os.getenv("MCP_SERVER_URL", "http://mcp:3003")
    ...
    async def initialize(self):
        """Initialize the MCP client session and verify connectivity."""
        self.session = aiohttp.ClientSession()
        self._initialized = True

    def is_available(self) -> bool:
        """Check if MCP client is available (initialized)."""
    ...
    async def call_tool(
        ...
        async with self.session.post(
            f"{self.bridge_url}/call_tool", json=payload
        ) as response:
            if response.status == 200:
                ...
                log_info(
                    self.logger, f"Tool call failed: {response.status} - {error_text}"
                )
    ...
    async def get_context(self, query: str, max_tokens: int = 128000) -> str:
        ...
        result = await self.call_tool(
            "context7", "get_context", {"query": query, "max_tokens": max_tokens}
        )
        if "error" in result:
    ...
    async def store_memory(self, key: str, value: str) -> bool:
        ...
        result = await self.call_tool("memory", "store", {"key": key, "value": value})
        if "error" in result:
    ...
    async def retrieve_memory(self, key: str) -> str:
        ...
        result = await self.call_tool("memory", "retrieve", {"key": key})
        if "error" in result:
