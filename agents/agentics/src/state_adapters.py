...
from .models import CodeSpec, TestSpecification, ValidationResults
...
class StateToCodeGenerationStateAdapter(Runnable[State, CodeGenerationState]):
    ...
    def invoke(self, input: State, config=None) -> CodeGenerationState:
        ...
        return CodeGenerationState(
            ...
            manual_implementation_notes=refined_ticket.get(
    ...
    def invoke(self, input: CodeGenerationState, config=None) -> State:
        ...
        state: State = {
            ...
            "refined_ticket": {
                ...
                "acceptance_criteria": self._safe_getattr(
                    input, "acceptance_criteria", []
                ),
                "implementation_steps": self._safe_getattr(
                    input, "implementation_steps", []
                ),
                "npm_packages": self._safe_getattr(input, "npm_packages", []),
                "manual_implementation_notes": self._safe_getattr(
                    input, "manual_implementation_notes", ""
                ),
            },
            ...
            "existing_tests_passed": self._safe_getattr(
                input, "existing_tests_passed", 0
            ),
            ...
            "recovery_confidence": self._safe_getattr(
                input, "recovery_confidence", 100.0
            ),
...
class IntegrationInputAdapter(Runnable[Dict[str, Any], CodeGenerationState]):
    ...
    def invoke(self, input: Any, config=None) -> CodeGenerationState:
        ...
        input_dict = (
            asdict(input)
            if hasattr(type(input), "__dataclass_fields__")
            else dict(input)
        )
        # Filter out keys that are not valid CodeGenerationState fields
        valid_fields = set(defaults.keys())
        filtered_dict = {k: v for k, v in input_dict.items() if k in valid_fields}
        state_dict = {**defaults, **filtered_dict}
