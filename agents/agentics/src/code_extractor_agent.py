...
class CodeExtractorAgent(ToolIntegratedAgent):
    def __init__(self, llm_client):
        super().__init__(
            llm_client,
            [read_file_tool, list_files_tool, check_file_exists_tool],
            "CodeExtractor",
        )
    ...
    def load_stop_words(self):
        ...
        try:
            if os.path.exists(stop_words_file):
                ...
                log_info(
                    self.name,
                    f"Loaded {len(stop_words)} stop words from {stop_words_file}: {stop_words}",
                )
            else:
                ...
                log_info(
                    self.name,
                    f"Using {len(stop_words)} default stop words: {stop_words}",
                )
            return stop_words
    ...
    def is_test_file(self, file_path):
        ...
        log_info(
            self.name,
            f"File {file_path} is {'a test file' if is_test else 'not a test file'}",
        )
        return is_test
    ...
    def extract_identifiers(self, ticket):
        ...
        try:
            ...
            self.monitor.warning(
                f"Failed to load stop words: {str(e)}. Using default stop words."
            )
            stop_words = {
                "a",
                "an",
        ...
        filtered_identifiers = [
            id for id in identifiers if id.lower() not in stop_words and len(id) > 2
        ]
        log_info(
            self.name, f"Extracted and filtered identifiers: {filtered_identifiers}"
        )
    ...
    def is_content_relevant(self, content, identifiers):
        ...
        is_relevant = any(
            identifier.lower() in content_lower for identifier in identifiers
        )
    ...
    def extract_code_structure(self, content):
        ...
        structure = {"classes": [], "methods": [], "interfaces": [], "functions": []}
    ...
    def process(self, state):
        ...
        try:
            ...
            all_files = (
                files_result.get("files", []) if isinstance(files_result, dict) else []
            )
            log_info(self.name, f"Found {len(all_files)} files in project")
        except Exception as e:
            log_info(
                self.name,
                f"Failed to list files: {str(e)}, falling back to hardcoded files",
            )
            ...
            for file_path in all_files:
                ...
                try:
                    ...
                    if is_relevant:
                        ...
                        log_info(
                            self.name,
                            f"Added relevant file: {file_path} (test: {is_test})",
                        )
            ...
            if not relevant_code_files:
                log_info(
                    self.name, "No relevant code files found, adding default main.ts"
                )
                main_content = self.read_file_content("src/main.ts")
                if main_content:
                    code_structure = self.extract_code_structure(main_content)
            ...
            if not relevant_test_files:
                log_info(
                    self.name,
                    "No relevant test files found, adding default main.test.ts",
                )
                test_content = self.read_file_content("src/__tests__/main.test.ts")
                if test_content:
                    test_structure = self.extract_code_structure(test_content)
        ...
        log_info(
            self.name,
            f"Total relevant files found: {len(relevant_code_files) + len(relevant_test_files)}",
        )
        log_info(
            self.name,
