...
class CodeExtractorAgent(BaseAgent):
    ...
    def process(self, state):
        ...
        prompt = (
            ...
            f"For tasks involving an Obsidian plugin (e.g., adding a new command or feature to `TimestampPlugin`), "
            f"prioritize updating 'src/main.ts' (the main plugin file where commands are added in the `onload` method) "
            f"and 'src/__tests__/main.test.ts' (the test file with `describe('TimestampPlugin', ...)` blocks) if they exist and are relevant. "
