...
def create_mock_llama_client():
    """Create mock llama client with realistic responses."""
    mock_client = MagicMock(spec=llamaClient)
    mock_client.name = "llama"
...
def create_mock_service_manager():
    ...
    mock_manager.llama_reasoning = create_mock_llama_client()
    mock_manager.llama_code = create_mock_llama_client()
...
def patch_health_monitor():
    ...
    return patch(
        "src.circuit_breaker.get_health_monitor",
        return_value=create_mock_health_monitor(),
    )
...
def create_mock_composable_workflows():
    ...
    mock_workflows.issue_processing_workflow = create_mock_agent(
        "issue_processing_workflow"
    )
    mock_workflows.code_generation_workflow = create_mock_agent(
        "code_generation_workflow"
    )
    mock_workflows.integration_testing_workflow = create_mock_agent(
    ...
    mock_workflows.llm_reasoning = create_mock_llama_client()
    mock_workflows.llm_code = create_mock_llama_client()
...
def create_mock_code_spec():
    ...
    return CodeSpec(language="typescript", framework="obsidian", dependencies=["uuid"])


def create_mock_test_spec():
    """Create mock TestSpecification."""
    return TestSpecification(
        test_framework="jest", coverage_requirements=["lines > 80%", "functions > 85%"]
    )


def create_mock_validation_results():
    """Create mock ValidationResults."""
    return ValidationResults(success=True, errors=[], warnings=["Minor style issue"])
...
def create_mock_collaborative_generator():
    ...
    def mock_invoke(input, config=None):
        if isinstance(input, CodeGenerationState):
            return input.with_code("collaboratively generated code").with_tests(
                "collaboratively generated tests"
            )
        return input
...
def create_comprehensive_mock_context():
    ...
    def comprehensive_mock_context():
        ...
        mocks = {
            "src.agent_composer.AgentComposer": patch(
                "src.agent_composer.AgentComposer",
                return_value=create_mock_agent_composer(),
            ),
            "src.composable_workflows.ComposableWorkflows": patch(
                "src.composable_workflows.ComposableWorkflows",
                return_value=create_mock_composable_workflows(),
            ),
            "src.services.ServiceManager": patch(
                "src.services.ServiceManager",
                return_value=create_mock_service_manager(),
            ),
            "src.circuit_breaker.get_circuit_breaker": patch(
                "src.circuit_breaker.get_circuit_breaker",
                side_effect=create_mock_circuit_breaker,
            ),
            "src.circuit_breaker.get_health_monitor": patch(
                "src.circuit_breaker.get_health_monitor",
                return_value=create_mock_health_monitor(),
            ),
            "src.monitoring.get_monitor": patch(
                "src.monitoring.get_monitor",
                return_value=create_mock_performance_monitor(),
            ),
            "src.monitoring.structured_log": patch(
                "src.monitoring.structured_log",
                side_effect=create_mock_structured_logger,
            ),
        }
    ...
    for i, (actual, expected) in enumerate(zip(actual_calls, expected_calls)):
        if isinstance(expected, dict):
            ...
            for key, value in expected.items():
                assert actual[1].get(key) == value, (
                    f"Call {i}: expected {key}={value}, got {actual[1].get(key)}"
                )
        else:
            return scenarios.get(scenario, scenarios["success"])
