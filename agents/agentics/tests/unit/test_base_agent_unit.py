...
def test_base_agent_state_type():
    ...
    state = State(
        ...
        refined_ticket={"key": "value"},
        result={},
        generated_code="code",
        generated_tests="tests",
        existing_tests_passed=1,
        existing_coverage_all_files=50.0,
        relevant_code_files=[{"file_path": "a.ts", "content": "a"}],
        relevant_test_files=[{"file_path": "a.test.ts", "content": "test"}],
    )
...
def test_base_agent_circuit_breaker_protection():
    ...
    with (
        patch.object(agent.circuit_breaker, "_record_success") as mock_success,
