...
def state_with_packages():
    return {
        "refined_ticket": {
            "npm_packages": [{"name": "uuid"}, {"name": "nanoid"}, "lodash"]
        }
    }
...
@patch.dict(os.environ, {"PROJECT_ROOT": "/project"})
def test_process_with_packages(
    agent, mock_tool_executor, state_with_packages
):
