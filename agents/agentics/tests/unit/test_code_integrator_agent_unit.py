...
def test_code_integrator_agent_update_files(temp_project_dir):
    ...
    # Mock LLM responses for code and test file updates by patching the class method
    with patch.object(type(agent.llm), 'invoke', side_effect=[
        "function main() {}\nfunction newFunc() {}",  # Response for code file
        "test('main', () => {})\ntest('newFunc', () => {})"  # Response for test file
    ]) as mock_invoke:
        # When: The agent processes the state (with mocked LLM responses)
        result = agent(state)
