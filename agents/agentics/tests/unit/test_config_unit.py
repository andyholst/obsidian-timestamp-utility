...
class TestAgenticsConfig:
    ...
    def test_agentics_config_with_valid_env_vars(self, mock_env_vars):
        ...
        assert config.llama_reasoning_model == "qwen3.6-35b-a3b"
        assert config.llama_code_model == "qwen3.6-35b-a3b"
...
class TestGlobalConfig:
    ...
    def test_get_config_uninitialized(self, clean_config, mock_env_vars):
        """Test get_config initializes config when not initialized."""
        # get_config should now auto-initialize instead of raising
        result = get_config()
        assert isinstance(result, AgenticsConfig)
