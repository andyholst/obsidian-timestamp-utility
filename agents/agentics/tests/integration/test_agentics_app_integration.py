...
in realistic scenarios with real services (llama, GitHub, MCP) where available.
...
# B17: live-service integration tests. They exercise AgenticsApp against REAL llama + GitHub.
# llama is a required LOCAL service; skip cleanly when LLAMA_HOST is absent. GitHub reads of the
...
class TestAgenticsAppIntegration:
    ...
    async def test_initialize_success(self):
        ...
        config = AgenticsConfig(
            ...
            llama_code_model=os.environ.get("LLAMA_CODE_MODEL", "qwen3.6-35b-a3b"),
        )
    ...
    async def test_agentics_app_initialization_with_real_services(self, agentics_app):
        ...
        assert agentics_app.service_manager.llama_reasoning is not None
        assert agentics_app.service_manager.llama_code is not None
        ...
        assert "llama_reasoning" in health_status
        assert "llama_code" in health_status
    ...
    async def test_service_health_monitoring_integration(self, agentics_app):
        ...
        expected_services = ["llama_reasoning", "llama_code", "github"]
