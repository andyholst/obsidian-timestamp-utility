...
3. **Workflow Error Propagation/CB**
...
11. **Composer + State + ToolAgent + Tools**
...
- **Run**: `pytest agents/agentics/tests/integration/ -m integration -v`; manual only.
...
- **Current Coverage**: Existing integration tests (e.g., [`test_phase1_core_infrastructure_integration.py`](agents/agentics/tests/integration/test_phase1_core_infrastructure_integration.py)) cover some E2E but lack detailed component isolation (e.g., no specific `with_code` chain tests, limited ToolExecutor multi-tool).
- **Missing**:
