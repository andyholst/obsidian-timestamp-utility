...
def _repo_root() -> str:
    ...
    # (1) git toplevel — but SKIP if src/main.ts starts with "// Generated TypeScript code"
    #     (that means /app is a shadowed mount from agents/agentics/src, not the real plugin root).
    try:
        out = _sp.run(
        ...
        if out.returncode == 0 and out.stdout.strip():
            ...
            ts_path = os.path.join(cand, "src", "main.ts")
            if os.path.isdir(os.path.join(cand, "openspec", "changes")) and os.path.isfile(ts_path):
                # Reject shadowed mounts: if main.ts starts with LLM-generated stub, skip it
                with open(ts_path, "r") as f:
                    first_line = f.readline()
                if not first_line.startswith("// Generated TypeScript code"):
                    return cand
    except Exception:
        pass
    ...
    candidates = ["/project"]  # Unshadowed repo root — checked first to beat shadowed mounts
    here = os.path.dirname(os.path.abspath(__file__))
    ...
    for _ in range(8):
        ...
        cur = parent
    candidates += ["/app", os.getcwd()]
    best = None
    for c in candidates:
        if c and os.path.isdir(os.path.join(c, "openspec", "changes")):
            has_main_ts = os.path.isfile(os.path.join(c, "src", "main.ts"))
            # Prefer candidates with package.json (plugin root) over pure Python mounts
            has_package_json = os.path.isfile(os.path.join(c, "package.json"))
            if has_main_ts and has_package_json:
                return c  # exact match: plugin root with both TS contract and build config
...
def _snapshot_repo_ts(root: str) -> dict:
    ...
    Also snapshots agents/agentics/src/main.ts and __tests__/main.test.ts so the B3 check
    can distinguish pre-existing files from pollution generated by this e2e run.
    """
    snap = {}
    for rel in ("src/main.ts", "src/__tests__/main.test.ts",
                "agents/agentics/src/main.ts", "agents/agentics/__tests__/main.test.ts"):
...
def test_change_driven_modal_generation_is_persistent_and_runnable():
    ...
    for rel in ("main.ts", "__tests__/main.test.ts"):
        abs_path = os.path.join(root, "agents", "agentics", "src", rel)
        agentics_key = os.path.join("agents", "agentics", "src", rel)  # matches snapshot key
        if os.path.isfile(abs_path) and agentics_key not in snapshot:
            polluted.append(abs_path)
    assert not polluted, (
