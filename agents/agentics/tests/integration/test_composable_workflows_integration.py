...
def sentence_model():
    ...
    return SentenceTransformer(
        "all-MiniLM-L6-v2", cache_folder="/tmp/.cache/huggingface"
    )
...
class TestComposableWorkflowsIntegration:
    ...
    def test_composable_workflows_initialization(
        self, composable_workflow, sentence_model
    ):
    ...
    def test_issue_processing_workflow_phase(self, composable_workflow):
        ...
        result = composable_workflow.issue_processing_workflow.invoke(
            {"url": issue_url}
        )
    ...
    def test_code_generation_workflow_phase(self, composable_workflow):
        ...
        # Verify code and test generation - result may be dict or CodeGenerationState
    ...
    def test_integration_testing_workflow_phase(self, composable_workflow):
        ...
        input_state = {
            ...
            "relevant_code_files": [],
            "relevant_test_files": [],
            "refined_ticket": {"title": "Test", "requirements": [], "acceptance_criteria": []},
            "result": {"title": "Test", "description": "Test description"},
        }

        result = composable_workflow.integration_testing_workflow.invoke(input_state)

        # Verify integration results - result may be dict or CodeGenerationState
    ...
    def test_workflow_error_handling(self, composable_workflow):
        ...
        # Workflow handles errors gracefully - returns result with error info
        result = asyncio.run(composable_workflow.process_issue(issue_url))
        assert result is not None
    ...
    def test_workflow_phase_isolation(self, composable_workflow):
        ...
        code_result = composable_workflow.code_generation_workflow.invoke(
            {"refined_ticket": {}}
        )
        assert code_result is not None

        # Test integration phase - provide all required fields
        integration_result = composable_workflow.integration_testing_workflow.invoke(
            {
                "generated_code": "```typescript\nexport class TestClass {}\n```",
                "generated_tests": "```typescript\ndescribe('TestClass', () => {});\n```",
                "refined_ticket": {"title": "Test", "requirements": [], "acceptance_criteria": []},
    ...
    def test_workflow_state_persistence_simulation(self, composable_workflow):
        ...
        assert result is not None
        expected_fields = ["refined_ticket", "generated_code", "generated_tests"]
    ...
    def test_lcel_parallel_agent_simulation(self, dummy_state):
        ...
        # Parallel execution should complete in roughly 0.1s, not 0.2s sequential
        assert duration < 0.5, f"Parallel execution took too long: {duration:.2f}s (expected <0.5s)"
