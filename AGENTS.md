...
- LangGraph-based workflow: 7 nodes — fetch → clarify → plan → extract → generate_code_tests → test → output.
- **Text→Pseudocode→Code pipeline**: LLM converts issue text to pseudocode, then deterministic 1:1 mapping constructs TypeScript (no LLM in code construction).
- **Deterministic test generation**: Tests generated from the function name (no LLM hallucination).
- Self-correction loop: eval gate with 7-criteria scoring, 3 retry attempts. `compiles_successfully` is NOT a hard gate.
- Single State TypedDict throughout — no adapter layers.
- E2E tests target real issues (#20, #22) with real Ollama + GitHub API.
- Unit tests: 413 tests covering nodes, helper functions, edge cases.
...
### Key Source Files
- `src/workflow.py` — `AgenticsWorkflow` class: LangGraph `StateGraph` with 7 nodes, text→pseudocode→code pipeline.
- `src/agentics.py` — `AgenticsApp`: service init, workflow creation, `process_issue()` API.
- `src/state.py` — `State` TypedDict (single state type for entire workflow, 26+ fields).
...
- `src/loop_engineering.py` — `verify_and_retry()`, `verify_generated_code()`, verification functions.
...
- **Hard gates**: code-test consistency (imports match exports) and `tests_pass==0.0` → total = 0.
- `compiles_successfully` (tsc --project tsconfig.json) is **NOT** a hard gate — contributes 0.25 to weighted score.
- Threshold: 0.4. Failed outputs retry up to 3 times with `eval_failure_context` fed back to LLM.
