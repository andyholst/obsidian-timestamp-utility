# Capability: uuid-v7-modal

Generate a timestamp-based UUID v7 and insert it at the cursor via a new Obsidian command.
...
- `npm test` (jest) passes for `main.test.ts`, asserting UUID v7 format, version/variant bits, cursor insertion, and the no-active-editor Notice path.
- The new command appears in the plugin's registered commands.
