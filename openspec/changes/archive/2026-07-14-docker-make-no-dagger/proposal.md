...
OpenSpec tasks and uses **Ollama** to generate the TS code and TS tests. MCP is dropped
...
- Delete all Dagger-only Makefile targets: `install-dagger`, `sync-pipeline-deps`,
  ...
  `start-mcp-persist`, `stop-mcp-persist`, and the socat/Ollama-proxy vars.
...
- `run-agentics` takes `CHANGE=<name>`: it runs `python -m prod.agentics openspec:<name>`,
  ...
  **Ollama** (`OLLAMA_HOST`, `OLLAMA_MODEL`) to generate `src/main.ts` + `src/__tests__/main.test.ts`.
...
- `docker-make-pipeline`: All build/test/lint/agentic Makefile targets run via
  ...
  Dagger** and **no MCP**. `run-agentics CHANGE=<name>` generates TS via Ollama from the
...
- The companion change `uuid-modal-agentic-generation` exercises `run-agentics CHANGE=...`
  end-to-end (Ollama generates the TS code + tests).
