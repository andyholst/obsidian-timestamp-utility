# Capability: docker-make-pipeline

All build, test, lint, and agentic Makefile targets run via `docker compose run`
against `docker-compose-files/*.yaml`, with **no Dagger** and **no MCP**.
...
- `openspec validate docker-make-no-dagger` passes.
- `make build-app` and `make test-app` succeed with no `bin/dagger` present.
- `make run-agentics CHANGE=uuid-modal-agentic-generation` runs locally end-to-end.
- No `MCP_SERVER_URL` / `mcp-bridge` references remain in tracked files.
