...
- Whether each integration test is hermetic (mocked) vs live (needs `GITHUB_TOKEN`/`LLAMA_HOST`)
...
## What Changes
1. **Audit + categorize** every integration test into: hermetic / live-llama / live-GitHub / dead.
...
3. **Hermeticize** valid-but-live tests where the live call is not the assertion (mock GitHub/llama).
4. **New Makefile gate** `loop-integration` inserted as step 2.5 of `loop-harness` (runs the
   integration suite; live tests skip cleanly without `GITHUB_TOKEN`/`LLAMA_HOST`).
