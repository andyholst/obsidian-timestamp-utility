...
- Integration/e2e tests also `@patch`/`monkeypatch` llama/HTTP, so they never
...
2. A **refactor of the agentic tests** so unit tests test the *real* logic of the
   ...
   assembly, UUID/prompt construction) and integration/e2e tests make **real** llama
   calls (no mocks) — they must actually hit `LLAMA_HOST` and validate generated TS.
...
- `agents/agentics/tests/unit/*`: refactor so unit tests test the **real** modules
  ...
  llama/LLM HTTP, network, FS boundaries) — never to replace the unit under test.
- `agents/agentics/tests/integration/*` + e2e: **remove** `@patch`/`monkeypatch` on
  llama/GitHub; make **real** calls to `LLAMA_HOST` (running llama) and,
...
- `agentic-tests-real-logic`: After `run-agentics`, the agentic unit + integration/e2e
  ...
  deterministic units); integration/e2e tests MUST make **real** llama calls (no mocks),
