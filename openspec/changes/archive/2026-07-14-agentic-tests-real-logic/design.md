...
- `agents/agentics/tests/integration/*` + e2e also `@patch`/`monkeypatch` Ollama/GitHub,
...
1. **Unit refactor (real logic, mock external only).** For each deterministic module,
   ...
   - `code_generator` / `test_generator` prompt construction → real prompt build;
     mock ONLY the Ollama HTTP boundary.
   Rule: a mock may wrap `requests.get/post`, `Ollama(...).chat`, `open()` on a
   network path, or `subprocess` — never the function/class being tested.
2. **Integration/e2e = real calls.** Remove `@patch`/`monkeypatch` on Ollama and GitHub.
   Point tests at the real `OLLAMA_HOST` (the running Ollama) and, for issue-fetch
   tests, the real GitHub API (`GITHUB_TOKEN`). Assert on real generated TS. Keep the
   `pytest -m e2e` / integration markers; these require Ollama (and GitHub token) to run.
