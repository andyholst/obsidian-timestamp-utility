# loop-harness-verbose-output Specification

## Purpose
Keep `make loop-harness` (via `scripts/run-loop-harness.sh`) VERBOSE and live-streaming: each
stage's container/pytest/jest output is streamed to the terminal in real time (not silenced to a
log), with a start banner + periodic heartbeat on quiet stages, and a clear PASS/FAIL/timeout line.
The runner wraps `make <stage>` in `setsid script … | tee` so nerdctl's forced `--tty` gets a
console without the interactive-job-control SIGSTOP deadlock; the Makefile `$(call docker_run, …)`
helper self-provides a console for standalone `make` commands (script only when stdout is not a tty).
## Requirements
