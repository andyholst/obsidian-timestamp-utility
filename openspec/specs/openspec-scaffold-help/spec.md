...
## Purpose
Documents and verifies the help/usage of the OpenSpec scaffolding harness: `scripts/scaffold-openspec-change.sh`
has a full `usage()` (SYNOPSIS / ARGUMENTS / two worked EXAMPLES / RESULT / NOTES) shown via
`--help` and on a missing `--name`, and `make openspec-new` with no `NAME` prints that usage.
Verified end-to-end through `make` (grep-assertion on the usage text). No git commit/push (B4/B14).
## Requirements
