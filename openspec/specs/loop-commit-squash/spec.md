# loop-commit-squash Specification

## Purpose
Defines the single squashing-commit Makefile target (`make squash-commits`, formerly
`make commit`): collapse every WIP commit ahead of `main` into ONE thorough, file-grounded
Angular commit drafted by the project-manager Hermes CLI, never pushing (B14).

### Requirement: `make squash-commits` produces one thorough, file-grounded Angular commit vs `main`
The `squash-commits` Make target MUST, when invoked on the current branch:
...
- **WHEN** `make squash-commits` runs and the branch is N commits ahead of `main`
