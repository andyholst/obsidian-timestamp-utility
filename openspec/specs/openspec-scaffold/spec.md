...
## Purpose
Defines the reproducible harness for creating OpenSpec changes in this repo: `make openspec-new
NAME=<name>` (backed by `scripts/scaffold-openspec-change.sh`) invokes the real `openspec new
change` CLI (B15 — never hand-writes the directory), seeds `proposal.md` / `tasks.md` /
`specs/<capability>/spec.md` from a template, and runs `openspec validate` so the change is
green before implementation. No git commit/push (B4/B14).
## Requirements
