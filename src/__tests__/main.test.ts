...
describe('Integration: implementtimestampbaseduuidge command', () => {
    let plugin: any;
    let mockEditor: any;

    beforeEach(() => {
        // Mock plugin with commands array (avoids importing main.ts which needs Obsidian)
        mockEditor = {
            replaceSelection: jest.fn()
        };
        plugin = {
            commands: [
                {
                    ...
                    editorCallback: (editor: any, _ctx: any) => {
                        editor.replaceSelection('mocked-implementtimestampbaseduuidge');
                    }
                }
            ]
        };
    ...
    it('should register the implementtimestampbaseduuidge command', () => {
        const commands = plugin.commands || [];
        const cmd = commands.find((c: any) => c.id === 'implementtimestampbaseduuidge');
    ...
    it('should call editorCallback and replace selection', () => {
        const commands = plugin.commands || [];
        const cmd = commands.find((c: any) => c.id === 'implementtimestampbaseduuidge');
        expect(cmd).toBeDefined();
        cmd.editorCallback(mockEditor, {});
        expect(mockEditor.replaceSelection).toHaveBeenCalledWith(expect.any(String));
    });

    it('should import implementtimestampbaseduuidge from generated module in main.ts', () => {
        const fs = require('fs');
        const mainContent = fs.readFileSync('./src/main.ts', 'utf8');
        expect(mainContent).toContain("import { implementtimestampbaseduuidge } from './generated/implementtimestampbaseduuidge'");
    });
});
