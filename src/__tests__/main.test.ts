...
describe('TimestampPlugin', () => {
  ...
  describe('rename-filename-with-title command', () => {
    test('renames file with first heading title', async () => {
      ...
      if (command && typeof command.callback === 'function') {
        await command.callback()
        expect(mockApp.fileManager.renameFile).toHaveBeenCalledWith(mockFile, 'folder/my_awesome_title.md')
      } else {
    ...
    test('renames file to untitled if no heading is found', async () => {
      ...
      if (command && typeof command.callback === 'function') {
        await command.callback()
        expect(mockApp.fileManager.renameFile).toHaveBeenCalledWith(mockFile, 'folder/untitled.md')
      } else {
        throw new Error('rename-filename-with-title command is not properly defined')
      }
    })
  ...
  describe('insert-uuid-v7 command', () => {
    test('adds insert-uuid-v7 command', async () => {
      await plugin.onload()
      const command = mockCommands['insert-uuid-v7']
      expect(command).toBeDefined()
      expect(command.name).toBe('Insert UUID v7')
    })

    test('inserts UUID v7 into editor when editor exists', async () => {
      await plugin.onload()
      const command = mockCommands['insert-uuid-v7']
      expect(command).toBeDefined()
      if (command && typeof command.callback === 'function') {
