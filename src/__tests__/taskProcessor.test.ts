...
describe('processTasks', () => {
  ...
  test('throws error for invalid source folder', async () => {
    ...
    await expect(processTasks(mockApp, 'invalid/source', 'output')).rejects.toThrow(
      'Invalid source folder: invalid/source'
    )
  })
  ...
  test('throws error for invalid output folder', async () => {
    ...
    mockApp.vault.getAbstractFileByPath.mockReturnValueOnce(mockSourceFolder).mockReturnValueOnce(null)
    await expect(processTasks(mockApp, 'source', 'invalid/output')).rejects.toThrow(
      'Invalid output folder: invalid/output'
    )
  })
  ...
  test('throws error when source and output folders are the same', async () => {
    ...
    await expect(processTasks(mockApp, 'same', 'same')).rejects.toThrow(
      'Source folder and output folder must be different.'
    )
  })
  ...
  test('processes tasks and creates output files', async () => {
    ...
    mockApp.vault.getAbstractFileByPath.mockReturnValueOnce(mockSourceFolder).mockReturnValueOnce(mockOutputFolder)
