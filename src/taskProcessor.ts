...
export async function processTasks(
  ...
  for (const date of allDates) {
    ...
    if (existingTaskLines.length > 0) {
      ...
      await app.vault.modify(existingFile instanceof obsidian.TFile ? existingFile : await app.vault.create(filePath, ''), (content) => content)
    } else if (existingFile) {
      await app.vault.delete(existingFile as obsidian.TFile)
    }
  }
}
