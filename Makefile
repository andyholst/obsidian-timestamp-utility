...
COMPOSE_CMD := ./containerd_wrapper.sh
...
build-image:
    $(COMPOSE_CMD) -f $(DOCKER_COMPOSE_FILE) build --build-arg REPO_NAME=$(REPO_NAME)

build-app: build-image
    $(COMPOSE_CMD) -f $(DOCKER_COMPOSE_FILE) run --rm -e TAG=$(TAG) -e REPO_NAME=$(REPO_NAME) app /bin/bash -c "\
...
build-generate-requirements:
    $(COMPOSE_CMD) build -t pip-requirements -f docker-files/pip-requirements/Dockerfile .

generate-requirements: build-generate-requirements
    $(COMPOSE_CMD) run --rm -v $(PWD)/agents/agentics:/app pip-requirements

build-image-agents:
    $(COMPOSE_CMD) -f $(DOCKER_COMPOSE_FILE_PYTHON) build

run-agentics: build-image-agents
    $(COMPOSE_CMD) -f $(DOCKER_COMPOSE_FILE_PYTHON) run --rm agentics

test-agents-unit: build-image-agents
    $(COMPOSE_CMD) -f $(DOCKER_COMPOSE_FILE_PYTHON) run --rm unit-test-agents

test-agents-integration: build-image-agents
    $(COMPOSE_CMD) -f $(DOCKER_COMPOSE_FILE_PYTHON) run --rm integration-test-agents
