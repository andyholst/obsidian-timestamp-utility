...
>    durable behaviours **B1–B18** that the loop must always honour.
>
...
Everything below (OpenSpec, the Python pipeline, the Makefile, B1–B18) is just a concrete
...
(B1–B18) — a permanent regression e2e that outlives every feature (B1–B6), the no‑commit/no‑push
...
| **Loop engineering** | *Correct* the generator across attempts until an objective gate passes | "How do I make the system converge on correct, and know when to stop?" | `make build-app`/`make test-app` gate + spec‑first self‑correct loop (bounded ~5) + durable behaviours B1–B18 + delivery step B12 |
...
├── AGENTS.md                         # Repo‑level authority: phase flow + B1–B18 summary
...
- **When Ollama is reachable** (normal dev box), `make test-agents-unit` MUST be run and its
  pass/fail reported alongside the mocked run.
- **When Ollama is unreachable**, `loop-unit-real` SKIPS (it must not error) — but the agent MUST
  say the real unit gate did *not* execute, not silently fold it into the mocked run's "green".
...
## 6. Durable behaviours B1–B18 (must always hold, never regress)
...
| **B17** | **Integration suite is a mandatory loop phase; no dead tests; live tests skip cleanly.** `make loop-harness` runs SEVEN stages: a collection guard (gate 0) followed by six gates (1–6): `loop-collect` (hermetic collection guard) → `loop-unit` (mocked, hermetic) → `loop-unit-real` (REAL agent unit tests on live Ollama, no mocks) → `loop-e2e` (B1/B3 e2e) → `loop-integration` (broad suite) → `loop-build-app` → `loop-test-app`. No dead duplicates; live tests `skipif` cleanly on `OLLAMA_HOST`. |
...
| **B20** | **NEVER declare a change "done" without running the loop gate first — hard pre-flight.** Before claiming any OpenSpec change complete (or "harness green/aligned/fixed"), run the gate and report real output. Preferred: `bash scripts/run-loop-harness.sh` (wrapper over `make loop-harness`, also `make loop-trigger`) — all seven stages. If the full run can't finish (live Ollama absent for `loop-e2e`/`loop-unit-real`, or npm build times out), STILL run the hermetic gates `make loop-collect` + `make loop-unit` (no external dep) — they MUST be green before any "done". Report honestly (PASS/SKIP/FAIL, failing stage named); never claim green if `loop-unit`/`loop-collect` is red. Fix root cause and re-run. (B4/B14: no git commit/push from the gate.) |
...
- Repo authority: `AGENTS.md` (phase flow + B1–B18 summary).
...
Never leave two of these describing different behaviour.
