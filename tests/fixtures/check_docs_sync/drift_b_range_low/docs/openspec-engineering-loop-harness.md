...
>    durable behaviours **B1–B28** that the loop must always honour.
>
...
Everything below (OpenSpec, the Python pipeline, the Makefile, B1–B28) is just a concrete
...
(B1–B28) — a permanent regression e2e that outlives every feature (B1–B6), the no‑commit/no‑push
...
| **Loop engineering** | *Correct* the generator across attempts until an objective gate passes | "How do I make the system converge on correct, and know when to stop?" | `make build-app`/`make test-app` gate + spec‑first self‑correct loop (bounded ~5) + durable behaviours B1–B28 + delivery step B12 |
...
├── AGENTS.md                         # Repo‑level authority: phase flow + B1–B28 summary
...
9. `loop-secret-scan-tests` — secret-scanner pytest suite (real gitleaks, fail-closed, containerized)
10. `check-docs-sync` — FINAL B8 gate: fails if any sync doc drifts on stage order / `loop-ts-floor` / B-range

"GREEN" = all ten stages PASS (or, where a live dependency is genuinely absent, skip cleanly — never a
...
## 6. Durable behaviours B1–B28 (must always hold, never regress)
...
- Repo authority: `AGENTS.md` (phase flow + B1–B28 summary).
