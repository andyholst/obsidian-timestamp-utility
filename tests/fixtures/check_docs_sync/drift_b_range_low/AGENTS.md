...
resort, B13). The durable behaviours B1–B28 are the loop's "laws of physics" — invariants that
...
- **The B8 sync docs are ONE source of truth — never let them drift.** The loop/harness
  ...
  2. **`AGENTS.md`** — this file: the authoritative narrative + the B1–B28 durable behaviours.
  ...
  loop-secret-scan-tests → check-docs-sync), the `loop-ts-floor` guard, the B1–B28 behaviours, and
