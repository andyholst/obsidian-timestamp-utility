...
(or `python3 scripts/run_loop_harness.py`) — i.e. the eleven-stage loop-harness run below. It runs **eleven stages in order** and must exit 0 on every
...
- **`nerdctl` + `docker compose run`** hardcodes `--interactive --tty`, so the container
  ...
  the PTY is provided by the caller — `scripts/run_loop_harness.py` wraps each `make <stage>` in
...
- **Request intake gate (front door) — turn inbound requests into OpenSpec changes before acting.**
  ...
  **start the worktree flow** — according to the **per-channel trigger**:
  ...
  `scripts/openspec_change_flow.py --name <name>`), which creates a dedicated local worktree `wt/<name>`,
